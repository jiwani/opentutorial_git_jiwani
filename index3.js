...
const session_bearer_key = '39cc0330-c85d-440e-9747-3fab0b45817c';
...
var getAccountId = function(agent) {
    ...
    agent.add(kindOfClouds + ' 계정에 account id 가 2개가 존재합니다. 원하시는 계정을 마우스로 선택하거나 숫자를 입력하세요(예: 1).');

    const accountInfo = { '1': '129335065553', '2': '216093335544', '3': null };
    agent.context.set({
        'name': 'app1',
        'lifespan': 1,
        'parameters': {
    ...
    const googlePayloadJson = {
        ...
        "items": [{
                ...
                "title": "Platform Service : 129335065553",
                "image": {
            ...
            },
            {
                ...
                "title": "prod-account : 216093335544",
                "image": {
                    "url": "http://www.boylesoftware.com/blog/wp-content/uploads/2015/04/checkmark.png",
            ...
            },
            {
                ...
                "image": {
                    "url": "http://www.boylesoftware.com/blog/wp-content/uploads/2015/04/checkmark.png",
                    "accessibilityText": "item image access 3"
                }
            }
        ]
...
var getCostByMonAccId = function(agent) {
    ...
    let accountId = app1.parameters.accountInfo[number];
    ...
    return request(options).then(function(parsedBody) {
        ...
        //        if(err.error.error == 'invalid_token') agent.add('invalid_token, 토큰값을 확인하세요.'); 
    });
}


var getAccountId2 = function(agent) {
    ...
    /***************** list_card를 이용한 요금 보기 끝 ******************/

var getCost_Month = function(agent) {
    const kindOfClouds = agent.parameters['kindOfClouds'];
    const dateTime = agent.parameters['date-time'];
    const costTerm = agent.parameters['cost'];
    ...
    return request(options).then(function(parsedBody) {
        ...
        /* 답변 생성 */
        let cost = parseFloat(parsedBody.Data[0].cost.toFixed(2));
        agent.add('알려드릴께요. [' + dateFormat(dateArg) + '] ' + ' 문의하신 ' + kindOfClouds + '의 ' + costTerm + '은 $' + numberWithCommas(cost) + '입니다.');

        agent.add('어카운트별로 ' + costTerm + '을 보시려면 하단을 클릭하세요.');
        agent.add(new Suggestion('어카운트별로 ' + ' ' + kindOfClouds + ' ' + dateFormat(dateArg) + ' ' + costTerm + '을 보여줘.'));
    }).catch(function(err) {
        console.log('err:', err);
        agent.add('값이 없습니다. 요청사항을 확인해 주세요.');
...
var getCostMonthAccount = function(agent) {

    const accountInfo = { '1': '129335065553', '2': '216093335544' };
    console.log('length:' + Object.keys(accountInfo).length);
    //    getCostMonthAccInfo(agent, accountInfo[i]);
    getCostMonthAccInfo(agent);
    //    for(let i=1; i <= Object.keys(accountInfo).length; i++){
    //    
    ////    agent.add('잠시만 기다려 주세요.');
    ////    	console.log('accountInfo:' + accountInfo[i]);
    //	return new Promise(function (resolve, reject){		
    //		resolve(getCostMonthAccInfo(agent, accountInfo[i]));
    //	})
    ////	.then(function (text){
    ////		console.log(text);
    ////	}, function(error){
    ...
    //	getCostMonthAccInfo(agent).then(function (text){
    //		console.log('text:' + text);
    //		agent.add(JSON.stringify(text));
...
var getCostMonthAccInfo = function(agent, accountId) {
    ...
    for (let i = 1; i <= Object.keys(accountInfo).length; i++) {
        ...
        return request(options).then(function(parsedBody) {
                ...
                let costTemp;
                if (parsedBody.Data[0] == undefined) costTemp = 0;
                else costTemp = parsedBody.Data[0].cost.toFixed(2);

                let costInfo = new Object();
                costInfo.name = accountId;
                costInfo.value = costTemp;
                costArray.push(costInfo);
                console.log('person:' + JSON.stringify(costArray));


                agent.add('OK, ' + JSON.stringify(costArray));
                return Promise.resolve(agent);
...
function WebhookProcessing(request, response) {
    ...
    //    intentMap.set('slot.in.getCost.month', getCostByMonth); // 계정별로 보기 list_card
    //    intentMap.set('slot.in.getCost.month_accountId', getCostByMonAccId); // 계정별로 보기 list_card
    intentMap.set('slot.in.getCost.month', getCost_Month);
    intentMap.set('slot.in.getCost.month.account', getCostMonthAccount);
